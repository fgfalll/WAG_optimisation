...
| [core/optimisation_engine.py:235-244](file:///d:/rep/4.6/co2eor_optimizer/core/optimisation_engine.py#L235-L244) | `calculate_co2_fvf_rb_per_mscf` raises `TypeError` due to keyword `p_psia` | Previously caught exception and fell back to `B_GAS_RB_PER_MSCF = 5.0` | Evaluate dynamic PR-EOS CO₂ FVF | Overestimated reservoir voidage of injected gas by 10× ($B_g = 5.0$ vs $0.50\text{ RB/MSCF}$) | **D** | **CRITICAL** | **RESOLVED**: Aligned method signature in `pvt_state.py` and call site; dynamic PR-EOS FVF evaluated cleanly with 0 fallbacks. |
