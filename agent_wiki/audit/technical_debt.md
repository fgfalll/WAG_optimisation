...
| **Data Management & Modals** | `ui/data_management_widget.py`, `ui/widgets/` | Parameter disconnections (viscosity typo, omitted Corey rel perm exponents), dead PVT table methods, and bounds validation keyword mismatch. See [**Data Management Audit**](data_management_audit.md). | **HIGH** | Fix viscosity keys, connect Corey rel perm inputs into payload, remove dead methods, and normalize `min_val`/`min` in `ParameterInputGroup`. |
| **Dialogs, Models, Utils & Widgets** | `ui/dialogs/`, `core/data_models.py`, `utils/`, `ui/widgets/` | Dead code (`ui/models/`, `geomechanics_3d_view.py`), 99 phantom fields in `EORParameters`, report generator duplicate blocks/key mismatch, UI deletion bug in `edit_uq_parameter_dialog.py`. See [**Full Audit**](dialogs_models_utils_widgets_audit.md). | **HIGH** | Connect dropped injection schemes in `data_integration_engine.py`, align report section keys, fix UI lifecycle bugs, and prune orphaned modules. |
