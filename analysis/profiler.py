...
class ProductionProfiler:
    ...
    def __init__(
        ...
        if self.reservoir.geostatistical_grid is not None:
            self.pore_volume_bbl = (
                self.reservoir.length_ft
                * self.reservoir.cross_sectional_area_acres
                * 43560
                * np.mean(self.reservoir.geostatistical_grid)
            ) / FT3_PER_BBL
        else:
            self.pore_volume_bbl = (
                self.reservoir.length_ft
                * self.reservoir.cross_sectional_area_acres
                * 43560
                * np.mean(self.reservoir.grid.get("PORO", 0.2))
            ) / FT3_PER_BBL
    ...
    def _calculate_geology_factor(self) -> float:
        ...
        if (
            hasattr(self.reservoir, "geostatistical_grid")
            and self.reservoir.geostatistical_grid is not None
        ):
            heterogeneity = self._calculate_heterogeneity_index()
            # Higher heterogeneity reduces sweep efficiency
            heterogeneity_penalty = 1.0 - (
                heterogeneity * 0.3
            )  # Up to 30% penalty for high heterogeneity
    ...
    def generate_all_profiles(self, ooip_stb: float, **kwargs) -> dict:
        ...
        if self.reservoir_fluid:
            try:
                ...
                logger.warning(
                    f"Could not calculate B_gas, falling back to default 5.0. Error: {e}"
                )
        ...
        enhanced_pi = (
            base_productivity_index * permeability_modifier * (1.0 - heterogeneity_index * 0.2)
        )
        ...
        well_status = np.full(
            project_life_days, "producer"
        )  # Track well status: 'producer' or 'injector'
        ...
        for day in range(project_life_days):
            ...
            s_gas = (
                ((np.sum(daily_co2_inj[:day]) - np.sum(daily_co2_prod_mscf[:day])) * B_gas)
                / self.pore_volume_bbl
                if self.pore_volume_bbl > 0
                else 0
            )
            s_water = (
                (np.sum(daily_water_inj[:day]) - np.sum(daily_water_prod_bbl[:day]))
                / self.pore_volume_bbl
                if self.pore_volume_bbl > 0
                else 0
            )
            ...
            if (
                Vp_ceff > EPSILON
                and (current_pressure + (inj_vol_rb / Vp_ceff)) > self.eor_params.max_pressure_psi
            ):
                ...
                if day % 100 == 0 or scaling_factor < 0.1:
                    logger.debug(
                        f"Day {day}: Pressure control activated. Scaling factor={scaling_factor:.4f}, Current pressure={current_pressure:.1f} psi"
                    )
            ...
            if self.eor_params.injection_scheme == "huff_n_puff":
                ...
                is_production_phase = (
                    daily_co2_inj[day] == 0 and day > 0
                )  # Not injection and not first day
                ...
                if is_injection_phase:
                    ...
                    pressure_increase = min(
                        pressure_increase, 100.0
                    )  # Increased from 50 to 100 psi/day
                    current_pressure += pressure_increase
                    logger.debug(
                        f"Day {day}: Huff-n-Puff injection - Pressure increase: {pressure_increase:.1f} psi"
                    )
                    ...
                    oil_production_volume = (
                        daily_oil_stb[day] * B_oil if day < len(daily_oil_stb) else 0
                    )
                    water_production_volume = (
                        daily_water_prod_bbl[day] * B_water
                        if day < len(daily_water_prod_bbl)
                        else 0
                    )
                    co2_production_volume = (
                        daily_co2_prod_mscf[day] * B_gas if day < len(daily_co2_prod_mscf) else 0
                    )
                    total_production_volume = (
                        oil_production_volume + water_production_volume + co2_production_volume
                    )

                    pressure_decline = (
                        (total_production_volume / Vp_ceff) if Vp_ceff > EPSILON else 0
                    )
                    # Ensure minimum pressure decline during production phases
                    pressure_decline = max(
                        pressure_decline, 5.0
                    )  # Minimum 5 psi/day decline during production
                    pressure_decline = min(
                        pressure_decline, 80.0
                    )  # Increased from 40 to 80 psi/day
                    current_pressure -= pressure_decline
                    logger.debug(
                        f"Day {day}: Huff-n-Puff production - Pressure decline: {pressure_decline:.1f} psi"
                    )
                else:
                    # Soaking phase: minimal pressure change
                    current_pressure -= (
                        2.0  # Small pressure decline during soaking (increased from 1.0)
                    )
            ...
            min_pressure = max(
                1600, self.initial_pressure * 0.35
            )  # Increased from 1500 to 1600, from 30% to 35% of initial
            current_pressure = np.clip(
                current_pressure, min_pressure, self.eor_params.max_pressure_psi
            )
            ...
            if well_status[day] == "producer":
                ...
                total_flow_rate_bpd = enhanced_pi * (
                    current_pressure - self.eor_params.wellbore_pressure
                )
                ...
                peak_production = enhanced_pi * (
                    self.initial_pressure - self.eor_params.wellbore_pressure
                )
                if day > ramp_up_days and total_flow_rate_bpd < (
                    peak_production * min_production_factor
                ):
                    total_flow_rate_bpd = peak_production * min_production_factor
                    if day % 100 == 0:
                        logger.debug(
                            f"Day {day}: Applying minimum production factor. Current pressure={current_pressure:.1f}, Production={total_flow_rate_bpd:.1f}"
                        )

                # --- Calculate fractional flows based on current saturations ---
                cumulative_inj_vol_rb_at_day = (np.sum(daily_co2_inj[:day]) * B_gas) + np.sum(
                    daily_water_inj[:day]
                )
                PVI_current = (
                    cumulative_inj_vol_rb_at_day / self.pore_volume_bbl
                    if self.pore_volume_bbl > 0
                    else 0
                )
                ...
                if self.eor_params.injection_scheme == "huff_n_puff":
                    ...
                    if is_production_phase:
                        ...
                        pore_volume_near_well = (
                            self.pore_volume_bbl * 0.4
                        )  # 40% of pore volume accessible
                        co2_saturation_near_well = min(
                            0.85, co2_volume_in_reservoir_rb / pore_volume_near_well
                        )
                        co2_saturation_near_well = np.clip(
                            co2_saturation_near_well, 0.4, 0.85
                        )  # Increased minimum from 0.3 to 0.4

                        # Calculate fractional flow based on actual CO2 saturation
                        f_co2_at_producer = self._fractional_flow(
                            np.array([co2_saturation_near_well])
                        )[0]
                        ...
                        cycle_efficiency = min(
                            0.95, base_efficiency + cycles_completed * improvement_rate
                        )  # Increased cap to 0.95
                        ...
                        f_co2_at_producer = min(
                            f_co2_at_producer, 0.85
                        )  # Increased cap from 0.8 to 0.85
                        ...
                        logger.debug(
                            f"Day {day}: Huff-n-Puff CO2 production - f_co2={f_co2_at_producer:.3f}, cycle={cycles_completed}"
                        )
                    ...
                    co2_cut_aggressiveness = (
                        0.8  # Reduced from 1.5 to make breakthrough more gradual
                    )
                    f_co2_at_producer = np.clip(
                        1.0 - np.exp(-co2_cut_aggressiveness * pvi_past_bt), 0, 0.6
                    )  # Reduced max from 0.8 to 0.6

                    # Also consider the CO2 saturation at the producer
                    # After breakthrough, CO2 saturation increases gradually
                    S_co2_at_producer = np.clip(
                        S_wf + (1 - self.eor_params.sor - S_wf) * (1 - np.exp(-0.3 * pvi_past_bt)),
                        S_wf,
                        1 - self.eor_params.sor,
                    )
                    # Calculate fractional flow based on actual CO2 saturation
                ...
                if day % 100 == 0 or (PVI_current >= PVI_bt and f_co2_at_producer > 0.1):
                    logger.debug(
                        f"Day {day}: PVI_current={PVI_current:.4f}, PVI_bt={PVI_bt:.4f}, f_co2_at_producer={f_co2_at_producer:.4f}"
                    )
                    logger.debug(
                        f"Breakthrough status: {'YES' if PVI_current >= PVI_bt else 'NO'}, df_dS_at_front={df_dS_at_front:.6f}"
                    )
                ...
                if PVI_current < PVI_bt:
                    ...
                    S_w_at_producer = np.clip(
                        self.eor_params.s_wc
                        + (1 - self.eor_params.s_orw - self.eor_params.s_wc)
                        * (1 - np.exp(-0.5 * pvi_past_bt)),
                        self.eor_params.s_wc,
                        1 - self.eor_params.s_orw,
                    )

                f_water_at_producer = self._water_fractional_flow(
                    np.array([S_w_at_producer]), current_pressure
                )[0]

                # Debug logging for water saturation
                if day % 100 == 0 or (PVI_current >= PVI_bt and f_water_at_producer > 0.1):
                    logger.debug(
                        f"Day {day}: PVI={PVI_current:.4f}, S_w={S_w_at_producer:.4f}, f_water={f_water_at_producer:.4f}"
                    )

                # --- Water Cut Shutdown Logic ---
                if f_water_at_producer > self.eor_params.water_cut_bwow:
                    logger.warning(
                        f"Day {day}: Water cut ({f_water_at_producer:.2f}) exceeds limit ({self.eor_params.water_cut_bwow:.2f}). Setting production to zero."
                    )
                ...
                # Enforce physical mass conservation: cumulative oil cannot exceed recoverable oil
                if cumulative_oil_produced + oil_rate_stb > recoverable_oil_stb:
                    oil_rate_stb = max(0.0, recoverable_oil_stb - cumulative_oil_produced)
        ...
        daily_pore_volumes_injected = (
            cumulative_inj_vol_rb / self.pore_volume_bbl
            if self.pore_volume_bbl > 0
            else np.zeros(project_life_days)
        )

        # --- Assemble final profiles ---
        # Use unified co2_recycling_fraction from eor_params
        recycling_fraction = getattr(self.eor_params, "co2_recycling_fraction", 0.9)
        daily_co2_recycled = daily_co2_prod_mscf * recycling_fraction
    ...
    def _resample_profile(
        self, daily_data: np.ndarray, resolution: str, key: str = None
    ) -> np.ndarray:
        ...
        is_state_variable = key and any(
            var in key.lower() for var in ["saturation", "temperature", "ratio", "efficiency"]
        )
    ...
    def _setup_injection_scheme(
        ...
        if is_wag or is_swag:
            ...
            logger.debug(
                f"WAG injection rates: CO2={co2_inj_mscf_per_day:.1f} MSCF/day, Water={water_inj_rate_bpd:.1f} bbl/day, WAG ratio={self.eor_params.wag_ratio:.2f}"
            )
        else:
        ...
        if is_wag:
            self._implement_wag_scheme(
                daily_co2_inj,
                daily_water_inj,
                co2_inj_mscf_per_day,
                water_inj_rate_bpd,
                project_life_days,
            )
        elif is_huff_n_puff:
            self._implement_huff_n_puff_scheme(
                daily_co2_inj,
                daily_water_inj,
                co2_inj_mscf_per_day,
                project_life_days,
            ...
            self._implement_tapered_scheme(
                daily_co2_inj, daily_water_inj, co2_inj_mscf_per_day, project_life_days
            )
        elif is_pulsed:
            self._implement_pulsed_scheme(
                daily_co2_inj, daily_water_inj, co2_inj_mscf_per_day, project_life_days
            )
        elif is_continuous:
    ...
    def _implement_wag_scheme(
        self,
        daily_co2_inj: np.ndarray,
        daily_water_inj: np.ndarray,
        co2_inj_rate_mscfd: float,
        water_inj_rate_bpd: float,
        project_life_days: int,
    ):
        """
        ...
        logger.debug(
            f"WAG scheme implemented: {current_day} days populated out of {project_life_days} total days"
        )
        logger.debug(
            f"WAG cycles: CO2 phase={co2_phase_days} days, Water phase={water_phase_days} days"
        )
        ...
        logger.debug(
            f"WAG injection pattern: CO2 days={co2_injection_days}, Water days={water_injection_days}"
        )

    def _implement_huff_n_puff_scheme(
        self,
        daily_co2_inj: np.ndarray,
        daily_water_inj: np.ndarray,
        co2_inj_rate_mscfd: float,
        ...
        logger.debug(
            f"Huff-n-Puff scheme implemented: {cycle_count} cycles completed, {current_day} days populated out of {project_life_days} total days"
        )
        logger.debug(
            f"Huff-n-Puff cycle structure: Inject={inject_days}d, Soak={soak_days}d, Prod={prod_days}d"
        )

    def _implement_swag_scheme(
        self,
        daily_co2_inj: np.ndarray,
        daily_water_inj: np.ndarray,
        co2_inj_rate_mscfd: float,
    ...
    def _implement_tapered_scheme(
        ...
        for day in range(min(taper_days, project_life_days)):
            ...
            if taper_function == "linear":
                ...
                multiplier = initial_multiplier - np.log1p(progress * 10) * (
