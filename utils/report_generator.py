...
class ReportGenerator(QObject):
    ...
    def __init__(self, units_manager: Optional[Any] = None):
        super().__init__()
        self.units_manager = units_manager
        self.wkhtml_path = self.find_wkhtmltopdf()

    def get_unit(self, category: str) -> str:
        """Get display unit string for a given physical category."""
        if self.units_manager is not None and hasattr(self.units_manager, "get_unit"):
            return self.units_manager.get_unit(category)
        defaults = {
            "flow_rate": "BOPD",
            "volume": "STB",
            "pressure": "psia",
    ...
    def find_wkhtmltopdf(self):
        try:
            ...
            self._log_diagnostic_info(
                "Please install the actual wkhtmltopdf binary from https://wkhtmltopdf.org/"
            )
            self._log_diagnostic_info(
                "The Python 'wkhtmltopdf' package is just a wrapper and doesn't include the binary."
            )
            return None
    ...
    def test_wkhtmltopdf_installation(self):
        try:
            ...
            result = subprocess.run(
                [self.wkhtml_path, "--version"], capture_output=True, text=True, timeout=10
            )
    ...
    def generate_report(
        self, report_data: Dict[str, Any], output_path: str, config: Dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            pdfkit.from_string(
                html_content, output_path, options=options, configuration=config_pdfkit
            )
            ...
            try:
                ...
                self.error_occurred.emit(
                    f"PDF generation failed: {str(e)}. Report saved as HTML: {html_output_path}"
                )
            except Exception as fallback_error:
                self.error_occurred.emit(
                    f"PDF generation failed: {str(e)} and HTML fallback also failed: {str(fallback_error)}"
                )
            return False
    ...
    def _generate_table_of_contents(self, config: Dict[str, Any]) -> str:
        ...
        if sections.get("decline_curve_analysis", True):
            toc_items.append(
                '<li><a href="#decline-curve-analysis">Decline Curve Analysis</a></li>'
            )
    ...
    def _generate_uncertainty_quantification(
        self, report_data: Dict[str, Any], config: Dict[str, Any]
    ) -> str:
    ...
    def _generate_decline_curve_analysis(
        self, report_data: Dict[str, Any], config: Dict[str, Any]
    ) -> str:
        ...
        <div id="decline-curve-analysis" class="section">
            ...
            <div class="subsection">
                ...
                <table class="data-table">
                    ...
                    <tr><td>Initial Rate (qi)</td><td>{parameters.get("qi", 0):.2f}</td><td>{self.get_unit("flow_rate")}</td></tr>
                    ...
                    <tr><td>Ultimate Recovery</td><td>{ultimate_recovery:,.0f}</td><td>{self.get_unit("volume")}</td></tr>
                    <tr><td>Peak Rate</td><td>{peak_rate:.2f}</td><td>{self.get_unit("flow_rate")}</td></tr>
                </table>
            </div>
            
            <div class="subsection">
                <h3>Forecast Summary</h3>
                <p>Decline curve analysis indicates an economic life of {economic_life:.1f} years with an ultimate recovery of {ultimate_recovery:,.0f} {self.get_unit("volume")}.</p>
    ...
    def _generate_validation_report(self, report_data: Dict[str, Any]) -> str:
        ...
        validation_report_str = report_data.get(
            "validation_report", "Validation report not available."
        )
        return f"""
