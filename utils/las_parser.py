...
try:
    ...
    class LASHeaderError(Exception): pass
...
def parse_las(file_path: str,
              ...
              engine: str = 'normal') -> Optional[WellData]:
    """
    ...
    Returns:
        Optional[WellData]: An object containing the parsed well data, or None if no valid data curves exist.

    Raises:
        IOError: If the LAS file cannot be read or found.
    ...
    if not Path(file_path).is_file():
        logging.error(f"LAS file not found: {file_path}")
        raise IOError(f"LAS file not found: {file_path}")

    try:
        # Attempt to read the LAS file using lasio
        las = lasio.read(str(file_path), engine=engine)
        logging.debug(f"Successfully read LAS file: {file_path}")
        logging.debug(f"Detected sections: {list(las.sections.keys())}")
    except LASHeaderError as e:
    ...
    if not well_data.validate():
        logging.warning(f"Data validation failed - inconsistent array lengths found in well '{well_name}'.")
        
    if len(well_data.properties) <= 1 and 'DEPT' in well_data.properties:
        logging.warning(f"LAS file for well '{well_name}' contains no data curves besides DEPT. Returning empty result.")
        return None
    
    logging.info(f"Successfully parsed {len(well_data.properties)} curves for well '{well_name}'.")
    return well_data
