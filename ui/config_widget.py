...
class ConfigWidget(QWidget):
    ...
    def __init__(self, config_manager: "ConfigManager", parent: Optional[QWidget] = None):
        ...
        self.INJECTION_PROFILE_DISPLAY_MAP: Dict[str, str] = ui_config.get(
            "injection_profile_display_map", {}
        )
        self.OBJECTIVES_DISPLAY_MAP: Dict[str, str] = ui_config.get("objectives_display_map", {})
        self.REVERSE_OIL_PROFILE_MAP: Dict[str, str] = {
            v: k for k, v in self.OIL_PROFILE_DISPLAY_MAP.items()
        }
        self.REVERSE_INJECTION_PROFILE_MAP: Dict[str, str] = {
            v: k for k, v in self.INJECTION_PROFILE_DISPLAY_MAP.items()
        }
        self.REVERSE_OBJECTIVES_MAP: Dict[str, str] = {
            v: k for k, v in self.OBJECTIVES_DISPLAY_MAP.items()
        }
    ...
    def _apply_changes(self):
        ...
        pending_data = {
            dc_name: asdict(instance) for dc_name, instance in self.config_instances.items()
        }
        ...
        for key, widget in self.input_groups.items():
            ...
            if scope in pending_data:
                ...
                if param_name == "oil_profile_type":
                    internal_value = self.REVERSE_OIL_PROFILE_MAP.get(raw_value, raw_value)
                elif param_name == "injection_profile_type":
                    internal_value = self.REVERSE_INJECTION_PROFILE_MAP.get(raw_value, raw_value)
                else:
        ...
        if self.operational_widgets:
            ...
            try:
                ...
                if enable_checkbox.isChecked():
                    ...
                    pending_data[op_instance_name]["target_objective_name"] = (
                        self.REVERSE_OBJECTIVES_MAP.get(display_name, "npv")
                    )
                    pending_data[op_instance_name]["target_objective_value"] = float(
                        self.operational_widgets["target_value_input"].value()
                    )
                else:
        ...
        if not all_valid:
            QMessageBox.critical(
                ...
                self.tr("Validation Error"),
                self.tr(
                    "One or more fields have invalid values. Please correct the highlighted fields."
                ),
            )
            return
        ...
        try:
            ...
            for name, data in pending_data.items():
                if name in self.default_instances:
                    ...
                    filtered_data = {
                        ...
                        if v is not None
                        or (
                            get_origin(dc_type.__annotations__.get(k)) in (Union, UnionType)
                            and type(None) in get_args(dc_type.__annotations__.get(k))
                        )
                    }
            ...
            QMessageBox.critical(
                self,
                self.tr("Validation Error"),
                self.tr("Could not apply changes due to inconsistent data:\n\n{}").format(e),
            )
            return
    ...
    def _create_eor_parameters_page(self, instance: Any, layout: QVBoxLayout):
        ...
        main_injection_fields = [
            ...
            "target_pressure_psi",
            "max_pressure_psi",
            "min_injection_rate_mscfd",
            "max_injection_rate_mscfd",
            "mobility_ratio",
            "density_contrast",
            "dip_angle",
            "interfacial_tension",
            "sor",
            "gas_oil_ratio_at_breakthrough",
            "water_cut_bwow",
            "co2_density_tonne_per_mscf",
            "injection_gor",
            "initial_gor",
            "default_mmp_fallback",
            "kv_kh_ratio",
        ]

        # Relative permeability parameters
        rel_perm_fields = ["s_gc", "n_o", "n_g", "s_wc", "s_orw", "n_w", "n_ow"]

        # Well control parameters
        well_control_fields = [
            "productivity_index",
            "wellbore_pressure",
            "well_shut_in_threshold_bpd",
            "max_injector_bhp_psi",
            "timestep_days",
        ]
        ...
        for field_name in rel_perm_fields:
            ...
            if field:
                self._create_input_group_for_field(
                    instance, field, rel_perm_layout, "EORParameters"
                )
        ...
        for field_name in well_control_fields:
            ...
            if field:
                self._create_input_group_for_field(
                    instance, field, well_control_layout, "EORParameters"
                )
    ...
    def _create_standard_page(
        self, instance: Any, layout: QVBoxLayout, exclude_fields: List[str] = None
    ):
        form_layout = QFormLayout()
        bounds_group = QGroupBox(self.tr("Optimizer Search Bounds (Optional)"))
        bounds_group.setToolTip(
            self.tr(
                "Override automatic search bounds. Leave fields blank to use dynamic, physically-based estimates."
            )
        )
        ...
        for field in fields(instance):
            ...
            if isinstance(field.type, type) and is_dataclass(field.type):
                ...
                for nested_field in fields(nested_instance):
                    self._create_input_group_for_field(
                        nested_instance, nested_field, nested_layout, instance_name
                    )
                ...
                for key, value in dict_instance.items():
                    ...
                    self._create_input_group_for_field(
                        dict_instance, fake_field, dict_layout, instance_name
                    )
                ...
                min_field = next(
                    (f for f in fields(instance) if f.name == min_bound_field_name), None
                )
                max_field = next(
                    (f for f in fields(instance) if f.name == max_bound_field_name), None
                )
                self._create_input_group_for_field(
                    instance, main_param_field, form_layout, instance_name
                )
                if min_field:
                    self._create_input_group_for_field(
                        instance, min_field, bounds_layout, instance_name
                    )
                    handled_fields.add(min_field.name)
                if max_field:
                    self._create_input_group_for_field(
                        instance, max_field, bounds_layout, instance_name
                    )
                    handled_fields.add(max_field.name)
                bounds_group.setVisible(True)
                handled_fields.add(main_param_field.name)
    ...
    def _create_algorithm_groupbox(self, name: str, instance: Any, parent_widget: QWidget):
        ...
        for field in fields(instance):
            self._create_input_group_for_field(
                instance, field, form_layout, type(instance).__name__
            )
    ...
    def _create_operational_page(self, op_instance: OperationalParameters, layout: QVBoxLayout):
        ...
        for field in fields(op_instance):
            if field.name not in exclude_from_general:
                self._create_input_group_for_field(
                    op_instance, field, general_layout, "Operational"
                )
        ...
        if eor_instance:
            ...
            injection_rate_field = next(
                (f for f in fields(eor_instance) if f.name == "injection_rate"), None
            )
            if injection_rate_field:
                self._create_input_group_for_field(
                    eor_instance, injection_rate_field, basic_injection_layout, "EORParameters"
                )

            target_pressure_field = next(
                (f for f in fields(eor_instance) if f.name == "target_pressure_psi"), None
            )
            if target_pressure_field:
                self._create_input_group_for_field(
                    eor_instance, target_pressure_field, basic_injection_layout, "EORParameters"
                )
        ...
        self._create_input_group_for_field(
            op_instance,
            next(f for f in fields(op_instance) if f.name == "recovery_model_selection"),
            recovery_form_layout,
            "Operational",
        )
        ...
        target_group = QGroupBox(self.tr("Optimizer Target Seeking"))
        target_group_layout = QVBoxLayout(target_group)
        enable_checkbox = QCheckBox(self.tr("Enable Target Seeking"))
        enable_checkbox.setToolTip(
            self.tr(
                "Check this to make the optimizer try to achieve a specific target value for an objective."
            )
        )
        ...
        objective_combo.addItems(self.OBJECTIVES_DISPLAY_MAP.values())
        ...
        target_controls_layout.addRow(self.tr("Target Objective:"), objective_combo)
        target_controls_layout.addRow(self.tr("Target Value:"), value_input)
        ...
        is_target_enabled = (
            op_instance.target_objective_name is not None
            and op_instance.target_objective_value is not None
        )
        ...
        recovery_model_widget = self.input_groups.get(
            f"{op_instance_name}.recovery_model_selection"
        )
    ...
    def _update_recovery_model_widgets(self):
        ...
        if not hasattr(self, "recovery_model_container"):
            return
        ...
        while self.recovery_model_layout.count():
            ...
            if widget:
                ...
                for k in keys_to_remove:
                    del self.input_groups[k]
                widget.deleteLater()

        op_instance_name = OperationalParameters.__name__
        model_selector_widget = self.input_groups.get(
            f"{op_instance_name}.recovery_model_selection"
        )
        if not model_selector_widget:
            return
        ...
        for param_name, param_value in model_params.items():
            ...
            kwargs = (
                {"input_type": "doublespinbox", "decimals": 4}
                if isinstance(param_value, float)
                else {"input_type": "spinbox"}
            )
            kwargs.update(
                {
                    "param_name": key,
    ...
    def _get_input_type_and_options(self, field: Field) -> dict:
        ...
        if origin in (Union, UnionType):
            base_type = next((t for t in get_args(base_type) if t is not type(None)), str)

        if field.name in self.SPECIAL_DROPDOWNS:
            kwargs["input_type"] = "combobox"
            kwargs["items"] = self.SPECIAL_DROPDOWNS[field.name]
        elif base_type is bool:
            kwargs["input_type"] = "checkbox"
        elif base_type is int:
            kwargs["input_type"] = "spinbox"
        elif base_type is float:
            kwargs["input_type"] = "doublespinbox"
        ...
        if "min" in field.metadata:
            kwargs["min_val"] = field.metadata["min"]
        if "max" in field.metadata:
            kwargs["max_val"] = field.metadata["max"]
        if "step" in field.metadata:
            kwargs["step"] = field.metadata["step"]
        if "decimals" in field.metadata:
            kwargs["decimals"] = field.metadata["decimals"]
        return kwargs

    def _create_input_group_for_field(
        self, instance: Any, field: Field, layout: QFormLayout, scope_name: str
    ):
        """Creates and configures a ParameterInputGroup for a given dataclass field."""
        key = f"{scope_name}.{field.name}"
        kwargs = self._get_input_type_and_options(field)

        current_value = getattr(instance, field.name)
        default_value = getattr(
            self.default_instances.get(type(instance).__name__), field.name, None
        )
        ...
        if field.name == "oil_profile_type":
            display_value = self.OIL_PROFILE_DISPLAY_MAP.get(current_value, current_value)
        elif field.name == "injection_profile_type":
            display_value = self.INJECTION_PROFILE_DISPLAY_MAP.get(current_value, current_value)
        elif isinstance(current_value, list):
            display_value = ", ".join(map(str, current_value))
        ...
        kwargs.update(
            {
                ...
                "default_value": display_value if current_value is not None else "",
                "help_text": help_text,
            }
        )
    ...
    def _confirm_reset_all(self):
        ...
        reply = QMessageBox.question(
            ...
            self.tr(
                "Reset all parameters to application defaults? This will discard any applied or pending changes."
            ),
    ...
    def _load_from_file(self):
        ...
        filepath, _ = QFileDialog.getOpenFileName(
            self, self.tr("Load Configuration File"), "", self.tr("JSON Files (*.json)")
        )
        if not filepath:
            return
        try:
            ...
            for dc_type in CONFIGURABLE_DATACLASSES.values():
                ...
                if instance_name in data and isinstance(data[instance_name], dict):
                    valid_data = {
                        k: v
                        for k, v in data[instance_name].items()
                        if k in {f.name for f in fields(dc_type)}
                    }
                    loaded_instances[instance_name] = dc_type(**valid_data)
                else:
                    loaded_instances[instance_name] = deepcopy(
                        self.default_instances[instance_name]
                    )
            ...
            if loaded_instances:
                ...
                self.config_source_label.setText(
                    self.tr("Loaded from File: {}").format(Path(filepath).name)
                )
            ...
            QMessageBox.critical(
                self, self.tr("Load Error"), self.tr("Could not load file:\n\n{}").format(e)
            )
    ...
    def _save_to_file(self):
        ...
        if self._is_dirty:
            QMessageBox.warning(
                ...
                self.tr("You have unapplied changes. Please apply or discard them before saving."),
            )
            return
        filepath, _ = QFileDialog.getSaveFileName(
            self, self.tr("Save Configuration As"), "config.json", self.tr("JSON Files (*.json)")
        )
        if not filepath:
            return
        try:
            ...
            QMessageBox.information(
                ...
                self.tr("Configuration saved to:\n{}").format(filepath),
            )
        except Exception as e:
            QMessageBox.critical(
                self, self.tr("Save Error"), self.tr("Could not save file:\n\n{}").format(e)
            )
    ...
    def _coerce_value(self, value: Any, type_hint: Type) -> Any:
        ...
        base_type = (
            next((t for t in args if t is not type(None)), type_hint) if is_optional else type_hint
        )
        if value is None or (isinstance(value, str) and not value):
            if is_optional:
                return None
            else:
                raise ValueError(self.tr("This field cannot be empty."))
        ...
        if base_origin is list or base_origin is List:
            ...
            try:
                ...
                raise ValueError(self.tr("Must be a comma-separated list of numbers."))
        if base_origin is tuple or base_type is tuple:
            if isinstance(value, tuple):
                return value
            try:
                return eval(str(value))  # Safely evaluate string " (0.5, 1.0) " into a tuple
            except Exception:
                raise ValueError(self.tr("Must be a valid tuple, e.g., (0.5, 1.0)"))
    ...
    def _update_profile_param_visibility(self):
        ...
        oil_profile_type_widget = self.input_groups.get(
            f"{profile_params_instance_name}.oil_profile_type"
        )
        if not oil_profile_type_widget:
            return
