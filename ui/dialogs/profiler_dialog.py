...
class ProfilerWidget(QWidget):
    ...
    def load_justification(self):
        ...
        html_file_path = os.path.join(current_dir, "..", "assets", "docs", "profiler_justification.html")
        self.justification_view.setUrl(QUrl.fromLocalFile(html_file_path))

    def run_tests(self):
        self.results_view.setHtml("<h1>Running Profiler Tests...</h1>")
        try:
            process = subprocess.run(["pytest", "tests/test_profiler.py", "-v", "-s"], capture_output=True, text=True, check=True, cwd="D:\\Стаття 2\\4.5\\co2eor_optimizer")
            output = process.stdout.replace("\n", "<br>")
            html = f"<pre>{output}</pre>"

            image_path = "D:\\Стаття 2\\4.5\\co2eor_optimizer\\profiler_results.png"
            if os.path.exists(image_path):
                with open(image_path, 'rb') as f:
                    encoded_image = base64.b64encode(f.read()).decode('utf-8')
                html += f'<h2>Test Plot</h2><img src="data:image/png;base64,{encoded_image}" alt="Profiler Results Plot">'
