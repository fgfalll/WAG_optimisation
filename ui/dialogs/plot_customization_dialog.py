...
class PlotCustomizationDialog(QDialog):
    ...
    def retranslateUi(self):
        ...
        self.setWindowTitle(self.tr("Customize Plot Appearance"))

        for key, display_name, _ in self.CUSTOMIZABLE_ELEMENTS:
            if key in self.form_labels:
                self.form_labels[key].setText(f"{self.tr(display_name)}:")

        for key, widget in self.input_widgets.items():
            if isinstance(widget, dict) and "button" in widget:
                widget["button"].setText(self.tr("Choose..."))
    ...
    def _choose_color(self, settings_key: str):
        ...
        if isinstance(color_widgets, dict):
            new_color = QColorDialog.getColor(color_widgets["color"], self, self.tr("Select Color"))
    ...
    def restore_defaults(self):
        ...
        QMessageBox.information(self, self.tr("Defaults Restored"), self.tr("Settings have been reset to defaults."))
