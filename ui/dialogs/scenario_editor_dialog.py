...
from PyQt6.QtCore import pyqtSignal, QEvent, QCoreApplication
...
try:
    from core.data_models import EconomicParameters, EORParameters, OperationalParameters
    ...
    logging.warning(
        "ScenarioEditorDialog: Could not import core data models or ParameterInputGroup. Functionality will be limited."
    )
...
class ScenarioEditorDialog(QDialog):
    ...
    def __init__(
        self, scenario_data: Dict, base_config: Dict[str, Any], parent: Optional[QWidget] = None
    ):
        super().__init__(parent)

        self.base_config = base_config
        self.base_values_map: Dict[str, Any] = {}

        self.current_data = deepcopy(scenario_data) or {
            "name": self.tr("New Scenario"),
            "params": {},
        }
    ...
    def _setup_ui(self):
        self.setWindowTitle(self.tr("Scenario Editor"))
        ...
        self.name_edit.setPlaceholderText(self.tr("Enter a unique name for this scenario"))
        self.name_edit.textChanged.connect(self._update_window_title)
        self.scenario_name_label = QLabel(self.tr("<b>Scenario Name:</b>"))
        name_layout.addRow(self.scenario_name_label, self.name_edit)
    ...
    def changeEvent(self, event: QEvent):
        """Handle language change event to dynamically update UI text."""
    ...
    def retranslateUi(self):
        ...
        self._update_window_title(self.name_edit.text())
        self.name_edit.setPlaceholderText(self.tr("Enter a unique name for this scenario"))
        self.scenario_name_label.setText(self.tr("<b>Scenario Name:</b>"))
    ...
    def _populate_all_tabs(self):
        if not DATACLASS_MAP or not ParameterInputGroup:
            error_label = QLabel(
                self.tr("Parameter editing disabled: core application modules not loaded.")
            )
            self.tab_widget.addTab(error_label, self.tr("Error"))
            return

        tab_titles = {
            "EconomicParameters": self.tr("Economic"),
            "EORParameters": self.tr("EOR"),
            "OperationalParameters": self.tr("Operational"),
        }
        for name, dc_type in DATACLASS_MAP.items():
            tab_title = tab_titles.get(name, name.replace("Parameters", ""))
    ...
    def _create_parameter_tab(self, title: str, dc_name: str, dc_type: Type):
        ...
        for field in fields(dc_type):
            ...
            kwargs = {
                ...
                "label_text": self.tr(field.name.replace("_", " ").title()),
                "default_value": display_value,
                "placeholder_text": self.tr("Base: {0}").format(base_value),
                "help_text": field.metadata.get(
                    "help", self.tr("Base value for this parameter is {0}.").format(base_value)
                ),
            }

            if field.name in self.SPECIAL_DROPDOWNS:
                kwargs.update(
                    {"input_type": "combobox", "items": self.SPECIAL_DROPDOWNS[field.name]}
                )
            else:
    ...
    def _on_parameter_updated(self, value: Any):
        ...
        try:
            ...
            if dc_name not in self.current_overrides:
                self.current_overrides[dc_name] = {}
    ...
    def save_and_accept(self):
        ...
        if not name:
            QMessageBox.warning(
                self, self.tr("Invalid Name"), self.tr("Scenario name cannot be empty.")
            )
            return
    ...
    def edit_scenario(data: Dict, base_config: Dict, parent: QWidget) -> Optional[Dict]:
        ...
        if not DATACLASS_MAP or not ParameterInputGroup:
            title = QCoreApplication.translate("ScenarioEditorDialog", "Error")
            message = QCoreApplication.translate(
                "ScenarioEditorDialog",
                "Cannot open Scenario Editor because core application components are missing.",
            )
