...
class SensitivityAnalysisWorker(QThread):
    ...
    def _on_progress(self, percent: int, message: str) -> None:
        """Callback for receiving progress updates from the analyzer."""
        if self._is_running:
    ...
    def run(self):
        ...
        self.progress_updated.emit(
            0, f"Starting Sensitivity Analysis: {method_title}..."
        )
        ...
        try:
            ...
            # FIX: Removed incorrect conditional logic. The worker now correctly
            # passes the arguments from the widget directly to the analyzer method
            # using keyword argument unpacking. This is cleaner, more robust, and
            # allows the analyzer methods to define their own signatures without
            # requiring changes in this worker.
            ...
            if self._is_running:
                self.error_occurred.emit(f"Configuration Error: {msg}")
            ...
            if not self._is_running:
                logger.info(f"SensitivityAnalysisWorker was stopped.")
