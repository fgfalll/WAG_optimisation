...
# OOIP values derived from SR3: cum_oil / recovery_factor
CMG_CASES = {
    "gmflu001_1D": {
        "name": "SPE3 Condensate Gas Cycling (1D)",
        "sr3_file": "cmg/flu/gmflu001_1D.sr3",
        "description": "Gas cycling case - OOIP from SR3: cum_oil=194,873, RF=0.7409",
        "reference": {
            "cumulative_oil": 194873.0,
            "recovery_factor": 0.7409,
    ...
    "gmflu001": {
        "name": "SPE5 Wasson Gas Cycling (3D)",
        "sr3_file": "cmg/flu/gmflu001.sr3",
        "description": "Gas cycling case - OOIP from SR3: cum_oil=2,254,086, RF=0.6535",
        "reference": {
            "cumulative_oil": 2254086.0,
            "recovery_factor": 0.6535,
            "final_pressure": 896.5,
        },
        "v_dp": 0.612,
        "permeability": 215.0,
        "ooip": 3449258.0,
    ...
    "gmflu002_1D": {
        ...
        "description": "CO2 flood 1D - OOIP from SR3: cum_oil=3,620,543, RF=0.7788",
        "reference": {
            "cumulative_oil": 3620543.0,
    ...
    "gmflu002": {
        ...
        "description": "CO2 flood 3D - OOIP from SR3: cum_oil=14,992,874, RF=0.3326",
        "reference": {
            "cumulative_oil": 14992874.0,
            "recovery_factor": 0.3326,
            "final_pressure": 1231.7,
        },
        "v_dp": 0.612,
        "permeability": 215.0,
        "ooip": 45076813.0,
    ...
    "gmflu003_1D": {
        ...
        "sr3_file": "cmg/flu/gmflu003_1D.sr3",
        "description": "H2/He variant - OOIP from SR3: cum_oil=57,987, RF=0.3123",
        "reference": {
            "cumulative_oil": 57987.0,
            "recovery_factor": 0.3123,
            "final_pressure": 1312.6,
        },
        "v_dp": 0.0,
        "permeability": 200.0,
        "ooip": 185699.0,
    ...
    "gmflu003": {
        ...
        "sr3_file": "cmg/flu/gmflu003.sr3",
        "description": "H2/He variant - OOIP from SR3: cum_oil=12,470, RF=0.0600",
        "reference": {
            "cumulative_oil": 12470.0,
            "recovery_factor": 0.0600,
            "final_pressure": 1111.6,
        },
        "v_dp": 0.612,
        "permeability": 215.0,
        "ooip": 207709.0,
...
def detect_timeseries_anomalies(
    ...
    for i in range(len(eng_rate)):
        ...
        if eng_rate[i] < threshold and eng_rate[i] < (mean_rate - 3 * std_rate):
            anomalies.append(
                f"Sudden drop in {name} at year {eng_t[i + 1]:.1f} (Rate: {eng_rate[i]:.2f}/yr)"
            )
    ...
    if cmg_t is not None and cmg_y is not None and len(cmg_t) > 2:
        ...
        if abs(diff[max_div_idx]) > div_threshold:
            anomalies.append(
                f"Major divergence from CMG in {name} at year {eng_t[max_div_idx]:.1f} (Diff: {diff[max_div_idx]:.2f})"
            )
...
def run_surrogate_simulation(case_id: str, cmg_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"execution_time": time.time() - start_time, "results": res}
...
def validate_single_case(case_id: str, plot_results: bool = False) -> ValidationResult:
    ...
    engine_data = {
        ...
        "final_pressure": res.get("pressure", [0])[-1]
        if isinstance(res.get("pressure", [0]), (list, np.ndarray))
        else res.get("pressure", 0),
    ...
    if isinstance(rf_cmg_final, (list, np.ndarray)):
        rf_cmg_final = rf_cmg_final[-1]
    ...
    if "recovery_profile" in cmg_data and isinstance(
        cmg_data["recovery_profile"], (list, np.ndarray)
    ):
        cmg_t_rf = (
            np.array(cmg_data.get("time_vector", engine_data["time_years"] * 365.25)) / 365.25
        )
    ...
    if "pressure_profile" in cmg_data and isinstance(
        cmg_data["pressure_profile"], (list, np.ndarray)
    ):
    ...
    anomalies.extend(
        detect_timeseries_anomalies(
            engine_data["time_years"],
            engine_data["rf_profile"],
            cmg_t_rf,
            cmg_rf,
            "Recovery Factor",
            is_pressure=False,
    ...
    return ValidationResult(
        ...
        metrics={
            "rf_rel_error": rf_rel_error,
            "oil_cumulative_error_pct": metrics["oil_cumulative_error_pct"],
        },
        passed=passed,
...
def generate_plots(case_id: str, cmg: dict, eng: dict):
    try:
        ...
        if "recovery_profile" in cmg and isinstance(cmg["recovery_profile"], (list, np.ndarray)):
            ...
            ax1.plot(
                cmg_t,
                np.array(cmg["recovery_profile"]) * 100,
                "b-",
                label="CMG Reference",
                linewidth=2,
            )
            ...
            ax1.axhline(
                y=cmg.get("recovery_factor", 0) * 100, color="b", linestyle="--", label="CMG Final"
            )
        ...
        if "rf_profile" in eng and len(eng_t) > 1:
            ...
            ax1.axhline(
                y=eng.get("recovery_factor", 0) * 100, color="r", linestyle="--", label="PhD Final"
            )
        ...
        if "pressure_profile" in cmg and isinstance(cmg["pressure_profile"], (list, np.ndarray)):
            ...
            ax2.axhline(
                y=cmg.get("final_pressure", 0), color="b", linestyle="--", label="CMG Final"
            )
        ...
        if "pressure_profile" in eng and len(eng_t) > 1:
            ...
            ax2.axhline(
                y=eng.get("final_pressure", 0), color="r", linestyle="--", label="PhD Final"
            )
