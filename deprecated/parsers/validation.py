...
from core.data_models import ReservoirData, WellData

def validate_reservoir_data(reservoir_data: ReservoirData) -> List[str]:
    """
    [DEPRECATED] Validates the ReservoirData object with context-aware checks for grid properties.
    Originally designed for validating 3D ECLIPSE / GRDECL block-centered grids.
    Deprecated as active validation is performed in core/data_integration_engine.py.
    """
    errors = []
    ...
    for prop_name, value in grid.items():
        ...
        if prop_name_upper in per_cell_props:
            ...
            if array.size not in [1, nx, total_cells]:
                errors.append(f"'{prop_name}' (per-X): size is {array.size}, expected {nx} or 1 or {total_cells}.")
        elif prop_name_upper in per_y_props:
            if array.size not in [1, ny, total_cells]:
                errors.append(f"'{prop_name}' (per-Y): size is {array.size}, expected {ny} or 1 or {total_cells}.")
        elif prop_name_upper in per_z_props:
            if array.size not in [1, nz, total_cells]:
                 errors.append(f"'{prop_name}' (per-Z): size is {array.size}, expected {nz} or 1 or {total_cells}.")
...
def validate_well_data(well_data: WellData) -> List[str]:
    """[DEPRECATED] Validate WellData object against schema and business rules"""
    errors = []
