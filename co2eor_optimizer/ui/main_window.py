...
# --- Project Imports ---
# This block handles all imports and provides a clear error message if any module is missing.
try:
    ...
    # Show a detailed error message box before exiting
...
class MainWindow(QMainWindow):
    ...
    between different modules. This class is the single source of truth for
    the currently active project's state.
    """
    ...
    def _setup_main_app_tabs_container(self):
        ...
        self.config_tab.save_configuration_to_file_requested.connect(self._on_save_standalone_config_requested)
        self.main_tab_widget.addTab(self.config_tab, QIcon.fromTheme("preferences-system"), "1. Configuration")

        # Tab 2: Data Management
        self.data_management_tab = DataManagementWidget(self)
        ...
        self.sensitivity_tab = SensitivityWidget(self)
        self.main_tab_widget.addTab(self.sensitivity_tab, QIcon.fromTheme("view-statistics"), "5. Sensitivity Analysis")
        
        self.uq_tab = UQWidget(self)
        self.main_tab_widget.addTab(self.uq_tab, QIcon.fromTheme("view-process-users"), "6. Uncertainty Quantification")
        
        self.ai_assistant_tab = AIAssistantWidget(self.app_settings, self)
        self.ai_assistant_tab.request_context_data.connect(self._provide_context_to_ai_assistant)
        self.main_tab_widget.addTab(self.ai_assistant_tab, QIcon.fromTheme("preferences-desktop-ai"), "7. AI Assistant")
        
        self.logging_tab = LoggingWidget(self)
        self.main_tab_widget.addTab(self.logging_tab, QIcon.fromTheme("text-x-generic"), "8. Application Logs")
    ...
    def _handle_start_new_project_action(self):
        if self._confirm_unsaved_changes():
            ...
            self._transition_to_main_app_view(focus_tab_index=1) # Focus Data Management
    ...
    def _perform_project_save(self, filepath: Path) -> bool:
        ...
        try:
            ...
            if save_project_to_tphd(data_to_save, filepath):
                self.set_project_modified(False)
                self.show_status_message(f"Project saved: {filepath.name}", 5000)
            ...
            QMessageBox.critical(self, "Save Project Error", f"Could not save project file:\n{filepath}\n\nError: {e}")
            return False
        finally:
            QApplication.restoreOverrideCursor()
    ...
    def _on_app_configurations_updated(self, new_configs: Dict[str, Any]):
        ...
        self.current_profile_params = new_configs.get(ProfileParameters.__name__, self.current_profile_params)
    ...
    def _on_optimization_run_completed(self, results: Dict[str, Any]):
        ...
        self.set_project_modified(True)
        self.show_status_message("Optimization complete.", 5000)
        if self.optimisation_engine_instance and self.optimisation_engine_instance.results:
            try:
                self.sensitivity_analyzer_instance = SensitivityAnalyzer(engine=self.optimisation_engine_instance)
                self.sensitivity_tab.update_analyzer(self.sensitivity_analyzer_instance)
                logger.info("SensitivityAnalyzer updated with new optimization results as base.")
            except Exception as e:
                logger.error(f"Error updating Sensitivity Analyzer post-optimization: {e}", exc_info=True)
    ...
    def _provide_context_to_ai_assistant(self, context_req: str):
        ...
        if self.current_reservoir_data:
            context_data['reservoir_summary'] = {
                'grid_dims': self.current_reservoir_data.runspec.get('DIMENSIONS') if self.current_reservoir_data.runspec else 'N/A',
