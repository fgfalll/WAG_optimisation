...
class HybridRecoveryModel(RecoveryModel):
    def __init__(self, **model_init_kwargs):
        ...
        te_config.update(model_init_kwargs) # Highest precedence for TE params
        self.transition_engine = TransitionEngine(
            mode=te_config.get('mode','sigmoid'), # Ensure .get for safety
            use_gpu=te_config.get('use_gpu', False), # Ensure .get for safety
        ...
        # Allow model_init_kwargs to provide complete dicts for sub-model params
...
class TransitionEngine:
    ...
    def enable_gpu_acceleration(self):
        if not CUPY_AVAILABLE:
            logging.warning("cupy not installed. GPU acceleration for TransitionEngine disabled.")
            self._gpu_enabled = False; return
        self._gpu_enabled = True
        logging.info("GPU acceleration for TransitionEngine enabled (conceptual).")
...
def recovery_factor(pressure: float, rate: float, porosity: float, mmp: float,
    ...
    selected_model_instance = constructor(**final_model_init_kwargs) if model_name_lower in ['miscible', 'immiscible', 'hybrid'] else constructor()
