...
# Create a unit registry for converting units
...
try:
    ...
    logging.critical("The 'thermo' library could not be imported. EOS models cannot function. "
                     f"Error: {str(e)}. Please verify installation with: pip install thermo")
    THERMO_AVAILABLE = False
    # Define dummy classes/functions to prevent import errors in other modules
    class DummyThermo: pass
    PR, SRK, FlashVLN = DummyThermo, DummyThermo, DummyThermo
...
P_STD_PA = ureg.Quantity(1, 'atm').to('Pa').magnitude
T_STD_K = ureg.Quantity(60, 'degF').to('K').magnitude
...
class EOSModel(ABC):
    ...
    def __init__(self, eos_params: EOSModelParameters):
        ...
        for i, name in enumerate(self.names):
            ...
            tc = ureg.Quantity(Tcs_in_R[i], 'degR').to('K').magnitude if Tcs_in_R[i] > 0 else chem.Tc
            if tc is None: raise ValueError(f"Could not determine Tc for {name}")
            self.Tcs.append(tc)

            # Critical Pressure
            pc = ureg.Quantity(Pcs_in_psia[i], 'psi').to('Pa').magnitude if Pcs_in_psia[i] > 0 else chem.Pc
        ...
        self.eos_mixture: Optional[Union[PR, SRK]] = None
    ...
    def _perform_flash_and_calc_props(self, T: float, P: float) -> Dict[str, Any]:
        ...
        if flash_obj.phase == 'L':
            results['oil_viscosity_cp'] = self.eos_mixture.viscosity_liquid(T, P, self.zi) * 1000.0
        ...
        # Single Gas Phase
            ...
            results['gas_viscosity_cp'] = self.eos_mixture.viscosity_gas(T, P, self.zi) * 1000.0
        ...
        # Two-Phase (Liquid-Vapor)
            ...
            results['oil_viscosity_cp'] = self.eos_mixture.viscosity_liquid(T, P, flash_obj.xs) * 1000.0
            ...
            results['gas_viscosity_cp'] = self.eos_mixture.viscosity_gas(T, P, flash_obj.ys) * 1000.0
...
class PengRobinsonEOS(EOSModel):
    ...
    def __init__(self, eos_params: EOSModelParameters):
        ...
        self.eos_mixture = PR(Tcs=self.Tcs, Pcs=self.Pcs, omegas=self.omegas, kijs=self.kijs, zs=self.zi, Vcs=self.Vcs, MWs=self.MWs, s_V=self.s_V)
    ...
    def calculate_properties(self, pressure_psia: float, temperature_F: float) -> Dict[str, Any]:
        ...
        try:
            T_K = ureg.Quantity(temperature_F, 'degF').to('K').magnitude
            P_Pa = ureg.Quantity(pressure_psia, 'psi').to('Pa').magnitude
...
class SoaveRedlichKwongEOS(EOSModel):
    ...
    def __init__(self, eos_params: EOSModelParameters):
        super().__init__(eos_params)
        self.eos_mixture = SRK(Tcs=self.Tcs, Pcs=self.Pcs, omegas=self.omegas, kijs=self.kijs, zs=self.zi, Vcs=self.Vcs, MWs=self.MWs, s_V=self.s_V)
