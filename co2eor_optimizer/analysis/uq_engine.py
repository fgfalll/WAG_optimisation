...
try:
    # This block ensures that the script can find the project's root directory
    # and add it to the system path, which is crucial for correct module imports.
...
# --- FIX: Decoupled all imports from the old global config_manager ---
# All necessary classes are now imported directly from their source modules.
try:
    ...
    # Define dummy classes to prevent the application from crashing on import failure.
    # The UI will be disabled, but the program can still start.
...
class UncertaintyQuantificationEngine:
    def __init__(self,
        ...
        Args:
            base_engine: An instance of OptimizationEngine. This engine provides
                         the baseline configuration and non-varying data.
            uq_settings: Dictionary of UQ settings. If None, it will be an empty dict.
                         The UI layer is now responsible for passing these settings.
        """
        if not isinstance(base_engine, OptimizationEngine):
            raise TypeError(f"UQ Engine requires a valid OptimizationEngine instance, but received {type(base_engine)}.")
        
        self.base_engine = base_engine
        self.uq_settings = uq_settings or {}
        ...
        for param_def in self.param_definitions:
            full_name = param_def.get("name")
            if not full_name:
                logger.warning(f"Skipping UQ parameter with no name: {param_def}")
                continue
            
            dist_type = param_def.get("distribution")
            dist_params = param_def.get("params")
            
            if not dist_type or not isinstance(dist_params, list):
                logger.warning(f"Skipping UQ parameter '{full_name}' due to missing/invalid distribution or params.")
                continue
           ...
           'matrix' in correlation_settings and 'param_order' in correlation_settings:
            try:
                ...
                if corr_matrix.shape != (len(marginal_distributions_list), len(marginal_distributions_list)):
                    ...
                    self.joint_distribution = chaospy.Nataf(chaospy.J(*marginal_distributions_list), corr_matrix)
    ...
    def _infer_scope_from_name(self, full_name: str) -> str:
        ...
        if hasattr(self.base_engine.economic_params, full_name): return 'economic'
    ...
    def _param_def_by_full_name(self, full_name: str) -> Optional[Dict[str, Any]]:
        return next((pd for pd in self.param_definitions if pd.get("name") == full_name), None)
    ...
    def run_mc_for_fixed_strategy(self,
        ...
        results_list = []
        base_econ_params = deepcopy(self.base_engine.economic_params)
        ...
        for i in range(n_samples):
            ...
            results = self.base_engine.evaluate_for_analysis(
                ...
                ooip_override=sampled_params_this_run.get('reservoir.ooip_stb'),
    ...
    def run_mc_for_optimization_under_uncertainty(self,
        ...
        if objectives_at_optimum is None: objectives_at_optimum = [self.base_engine.chosen_objective, 'final_recovery_factor_reported']

        # Get pristine base data models for deep copying in the loop
        base_reservoir_ref = self.base_engine._base_reservoir_data
        base_pvt_ref = self.base_engine._base_pvt_data
        base_eor_params_ref = self.base_engine._base_eor_params
        base_econ_params_ref = self.base_engine._base_economic_params
        base_op_params_ref = self.base_engine._base_operational_params
        base_profile_params_ref = self.base_engine._base_profile_params
        ...
        for i in range(n_samples):
            ...
            try:
                temp_engine = OptimizationEngine(
                    ...
                    # Pass other base params from the main engine
                    ga_params_instance=self.base_engine.ga_params_default_config,
                    bo_params_instance=self.base_engine.bo_params_default_config,
                    **engine_init_overrides
                )
    ...
    def run_polynomial_chaos_analysis(self,
        ...
        try:
            ...
            nodes_uq, weights_uq = chaospy.generate_quadrature(q_order, self.joint_distribution, rule=q_rule)
        ...
        for i in range(nodes_uq.shape[1]):
            ...
            results_at_node = self.base_engine.evaluate_for_analysis(
                eor_operational_params_dict=fixed_eor_params_dict,
                ...
                ooip_override=sampled_params_for_node.get('reservoir.ooip_stb'),
