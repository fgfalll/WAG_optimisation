...
# or remain None if imports fail. This helps Pylance understand the potential
...
try:
    ...
    logging.critical(f"SensitivityWidget: Failed to import PyQt6.QtWebEngineWidgets: {e}. Plotting features may be disabled.")
...
class ParameterSelectionTreeView(QTreeView):
    ...
    def populate(self, param_structure: Dict[str, List[Tuple[str, str]]]):
        ...
        for category, params in param_structure.items():
            ...
            for name, path in params:
                ...
                # Use Qt.ItemDataRole.UserRole for custom data
    ...
    def get_selected_paths(self) -> List[str]:
        ...
        for i in range(root.rowCount()):
            ...
            if cat_item is None: # Defensive check
                continue
            for j in range(cat_item.rowCount()):
                param_item = cat_item.child(j)
                if param_item is None: # Defensive check
                    continue
...
class SensitivityWidget(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        super().__init__(parent)
        # Using string literals for type hints for attributes
        # where the type might be conditionally imported or cause Pylance issues.
        # `from __future__ import annotations` makes this possible and is often
        # the most robust way to handle such scenarios with type checkers.
        self.analyzer: Optional['SensitivityAnalyzer'] = None
        self.worker: Optional['SensitivityAnalysisWorker'] = None
        # Explicitly declare these attributes here to help Pylance
        self.tornado_view: QWidget
        self.spider_view: QWidget
        self.contour_view: QWidget
        ...
        self.objectives_edit: QLineEdit
    ...
    def _create_config_panel(self) -> QWidget:
        ...
        # 2. Analysis Configuration Group
        ...
        self.variation_edit = QLineEdit()
        self.variation_edit.setPlaceholderText("e.g., 20% or 1000,1500,2000")
        config_grid.addWidget(self.variation_edit, 2, 1)

        config_grid.addWidget(QLabel("Objectives:"), 3, 0)
        self.objectives_edit = QLineEdit("npv, recovery_factor")
        self.objectives_edit.setPlaceholderText("Comma-separated objectives, e.g., npv, recovery_factor")
        config_grid.addWidget(self.objectives_edit, 3, 1)
    ...
    def _create_results_panel(self) -> QWidget:
        ...
        # Check if QWebEngineView was successfully imported
        if QWebEngineView:
            # If QWebEngineView is available, instantiate them
            self.tornado_view = QWebEngineView()
            self.spider_view = QWebEngineView()
            self.contour_view = QWebEngineView()
        else:
            # If QWebEngineView is not available, use QLabel placeholders
            self.tornado_view = QLabel("Plotting functionality (Tornado Plot) requires PyQt6-WebEngine. Please install it.")
            self.tornado_view.setAlignment(Qt.AlignmentFlag.AlignCenter)
            self.spider_view = QLabel("Plotting functionality (Spider Plot) requires PyQt6-WebEngine. Please install it.")
            self.spider_view.setAlignment(Qt.AlignmentFlag.AlignCenter)
            self.contour_view = QLabel("Plotting functionality (Contour Plot) requires PyQt6-WebEngine. Please install it.")
            self.contour_view.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        self.results_tabs.addTab(self.contour_view, "Contour Plot")
    ...
    def update_analyzer(self, analyzer: Optional[SensitivityAnalyzer]):
        """
        Public method for MainWindow to provide the SA analyzer instance.
        This method is called by the main application window to provide the
        SensitivityAnalyzer object, which is crucial for running analyses.
        """
    ...
    def _on_type_changed(self, sa_type: str):
        """
        Adjusts UI elements based on the selected analysis type.
        """
        # Only set tab visible if it's a QWebEngineView, otherwise it's a QLabel placeholder
        if isinstance(self.contour_view, QWebEngineView):
            self.results_tabs.setTabVisible(self.results_tabs.indexOf(self.contour_view), sa_type == "Two-Way")
        else:
            # If QWebEngineView is not available, the tab should probably remain hidden or show a message
            # For now, we'll just log a warning if someone tries to hide/show a non-existent web view.
            if sa_type == "Two-Way":
                logger.warning("Attempted to show Contour Plot tab, but PyQt6-WebEngine is not installed.")


        tooltips = {
            "One-Way": "e.g., 20% (percentage variation) OR 1000,1500,2000 (specific values) OR 1000,2000 (min,max range)",
            "Two-Way": "e.g., 10,20,30;0.5,0.6,0.7 (Param1 vals; Param2 vals). Requires exactly two selected parameters.",
            "Re-Optimization": "e.g., 1000,1500,2000 (specific values for the single selected parameter). Requires exactly one selected parameter."
        }
        self.variation_edit.setToolTip(tooltips.get(sa_type, "Enter variation values based on analysis type."))

        placeholders = {
            "One-Way": "e.g., 20% or 1000,1500,2000",
            "Two-Way": "e.g., 10,20,30;0.5,0.6,0.7",
    ...
    def _run_analysis(self):
        """
        Initiates the sensitivity analysis based on user inputs.
        """
        if not self.analyzer:
            QMessageBox.critical(self, "Analyzer Error", "Sensitivity Analyzer is not available. Please load an optimization engine first.")
            return
        ...
        objectives = [obj.strip() for obj in self.objectives_edit.text().split(',') if obj.strip()]

        if not selected_paths:
            QMessageBox.warning(self, "Input Error", "Please select at least one parameter for analysis.")
            return
        if not objectives:
            QMessageBox.warning(self, "Input Error", "Please specify at least one objective for analysis.")
            return
        ...
        # Ensure SensitivityAnalysisWorker is not None before instantiation
    ...
    def _prepare_run_args(self, sa_type: str, paths: List[str], objectives: List[str]) -> Tuple[str, Dict]:
        """
        Validates inputs and prepares arguments for the SensitivityAnalysisWorker.
        """
        ...
        if sa_type == "One-Way":
            if not variation_text:
                raise ValueError("Variation input cannot be empty for One-Way analysis.")
            return "run_one_way_sensitivity", {
                "param_paths": paths,
                "variation_str": variation_text,
                "num_steps": num_steps,
            ...
            if len(variations) != 2 or not all(v.strip() for v in variations):
                raise ValueError("Two-Way variation requires two comma-separated value lists separated by ';'.")
            ...
            if not variation_text:
                raise ValueError("Variation input cannot be empty for Re-Optimization analysis (specify values).")
            return "run_re_optimization_sensitivity", {
                "param_path": paths[0],
                "param_values_str": variation_text,
                "objectives": objectives,
            }
        else:
    ...
    def _clear_results(self):
        """Clears all displayed results (plots and data table)."""
        # Only clear web views if they are actually QWebEngineView instances
        if isinstance(self.tornado_view, QWebEngineView):
            self.tornado_view.setHtml("")
        if isinstance(self.spider_view, QWebEngineView):
            self.spider_view.setHtml("")
        if isinstance(self.contour_view, QWebEngineView):
            self.contour_view.setHtml("")
    ...
    def _on_result(self, df: pd.DataFrame):
        ...
        Populates the data table and generates plots.
        """
        self._clear_results()
        if df.empty:
    ...
    def _populate_data_table(self, df: pd.DataFrame):
        """Populates the QTableWidget with data from the pandas DataFrame."""
    ...
    def _generate_plots(self, df: pd.DataFrame):
        """
        Generates and displays plots based on the analysis type and results.
        Uses QWebEngineView to render Plotly figures if available.
        """
        # Only proceed with plotting if QWebEngineView is available and the views are actual QWebEngineView instances
        if not QWebEngineView or not isinstance(self.tornado_view, QWebEngineView):
            ...
            # Ensure placeholder messages are shown if plotting is not possible
            if not isinstance(self.tornado_view, QWebEngineView): # Only set if it's a QLabel
                ...
                self.spider_view.setText("<p style='color:red; text-align:center;'>Plotting functionality requires PyQt6-WebEngine, which is not installed.</p>")
                self.contour_view.setText("<p style='color:red; text-align:center;'>Plotting functionality requires PyQt6-WebEngine, which is not installed.</p>")
            return

        sa_type = self.sa_type_combo.currentText()
        objectives_list = [obj.strip() for obj in self.objectives_edit.text().split(',') if obj.strip()]
        primary_obj = objectives_list[0] if objectives_list else None
        ...
        if not primary_obj or primary_obj not in df.columns:
            ...
            plot_error_html = f"<p style='color:red; text-align:center;'>Cannot generate plots: Primary objective '{primary_obj}' not found in results data.</p>"
            self.tornado_view.setHtml(plot_error_html)
            self.spider_view.setHtml(plot_error_html)
            self.contour_view.setHtml(plot_error_html)
            return
        ...
        if sa_type == "One-Way":
            try:
                if self.analyzer:
                    ...
                    self.tornado_view.setHtml(tornado_fig.to_html(include_plotlyjs='cdn'))
                    self.spider_view.setHtml(spider_fig.to_html(include_plotlyjs='cdn'))
                    self.results_tabs.setCurrentWidget(self.tornado_view)
                    logger.info("One-Way sensitivity plots (Tornado, Spider) generated.")
                else:
                ...
                self.spider_view.setHtml(f"<p style='color:red;'>Plotting Error: {e}</p>")

        elif sa_type == "Two-Way":
            selected_paths = self.param_tree.get_selected_paths()
            if len(selected_paths) != 2:
                logger.error("Two-Way analysis requires exactly two selected parameters for plotting.")
                self.contour_view.setHtml("<p style='color:red;'>Error: Two-Way analysis requires exactly two selected parameters for contour plot generation.</p>")
                return
            param1_path, param2_path = selected_paths[0], selected_paths[1]
            try:
                if self.analyzer:
                    contour_fig = self.analyzer.plot_contour_chart(df, param1_path, param2_path, primary_obj)
                    self.contour_view.setHtml(contour_fig.to_html(include_plotlyjs='cdn'))
                    self.results_tabs.setCurrentWidget(self.contour_view)
                    logger.info("Two-Way sensitivity plot (Contour) generated.")
                else:
                    raise RuntimeError("Analyzer is not available to generate plots.")
            except Exception as e:
                logger.error(f"Error generating contour plot: {e}", exc_info=True)
                self.contour_view.setHtml(f"<p style='color:red;'>Plotting Error: {e}</p>")
        elif sa_type == "Re-Optimization":
            logger.info("Re-Optimization results received. Plotting for this type is not yet implemented.")
            # Ensure these are QWebEngineView instances before calling setHtml
            if isinstance(self.tornado_view, QWebEngineView):
                self.tornado_view.setHtml("<p style='color:blue; text-align:center;'>Re-Optimization results displayed in Data Table. Plotting functionality for this analysis type is under development.</p>")
            if isinstance(self.spider_view, QWebEngineView):
                self.spider_view.setHtml("<p style='color:blue; text-align:center;'>Re-Optimization results displayed in Data Table. Plotting functionality for this analysis type is under development.</p>")
            if isinstance(self.contour_view, QWebEngineView):
                self.contour_view.setHtml("<p style='color:blue; text-align:center;'>Re-Optimization results displayed in Data Table. Plotting functionality for this analysis type is under development.</p>")


    def _on_error(self, error_msg: str):
        """
        Slot to handle errors reported by the worker thread.
        Displays the error message in the data table.
        """
        self._clear_results()
        self.data_table.setColumnCount(1)
        self.data_table.setRowCount(1)
    ...
    def _on_worker_finished(self):
        """
        Slot called when the worker thread finishes, regardless of success or failure.
        Re-enables the run button and cleans up the worker.
        """
        self.run_button.setEnabled(self.analyzer is not None)
        if self.worker:
