...
class ProfilerUtils:
    ...
    def resample_profile(
        daily_data: np.ndarray, resolution: str, key: str = None, project_lifetime_years: int = None
    ) -> np.ndarray:
        """
    ...
    def calculate_pore_volume(reservoir) -> float:
        ...
        if reservoir.geostatistical_grid is not None:
            pore_volume_bbl = (
                reservoir.length_ft
                * reservoir.cross_sectional_area_acres
                * 43560
                * np.mean(reservoir.geostatistical_grid)
            ) / FT3_PER_BBL
        else:
            pore_volume_bbl = (
                reservoir.length_ft
                * reservoir.cross_sectional_area_acres
                * 43560
                * np.mean(reservoir.grid.get("PORO", 0.2))
            ) / FT3_PER_BBL
    ...
    def normalize_saturations(s_oil: float, s_gas: float, s_water: float) -> tuple:
        ...
        s_gas = max(
            s_gas, SATURATION_EPSILON
        )  # Allow gas to be effectively zero (FIX: no 1% floor)
    ...
    @staticmethod
    def calculate_enhanced_productivity_index(
        base_pi: float, permeability_modifier: float, heterogeneity_index: float
    ) -> float:
        """
    ...
    @staticmethod
    def calculate_huff_n_puff_co2_production(
        total_injected_co2: float,
        total_produced_co2: float,
        pore_volume_bbl: float,
        B_gas: float,
    ...
    def assemble_final_profiles(
        ...
        daily_injector_permeability: np.ndarray = None,
    ) -> Dict[str, np.ndarray]:
        """
        ...
        profiles = {
            ...
            "yearly_oil_stb": ProfilerUtils.resample_profile(
                daily_oil_stb, "yearly", "oil_stb", int(len(daily_oil_stb) / DAYS_PER_YEAR)
            ),
            "yearly_co2_injected_mscf": ProfilerUtils.resample_profile(
                daily_co2_inj, "yearly", "co2_injected", int(len(daily_co2_inj) / DAYS_PER_YEAR)
            ),
            "yearly_water_injected_bbl": ProfilerUtils.resample_profile(
                daily_water_inj,
                "yearly",
                "water_injected",
                int(len(daily_water_inj) / DAYS_PER_YEAR),
            ),
            "yearly_water_produced_bbl": ProfilerUtils.resample_profile(
                daily_water_prod_bbl,
                "yearly",
                "water_produced",
                int(len(daily_water_prod_bbl) / DAYS_PER_YEAR),
            ),
            "yearly_co2_produced_mscf": ProfilerUtils.resample_profile(
                daily_co2_prod_mscf,
                "yearly",
                "co2_produced",
                int(len(daily_co2_prod_mscf) / DAYS_PER_YEAR),
            ),
            "yearly_co2_purchased_mscf": ProfilerUtils.resample_profile(
                daily_co2_purchased,
                "yearly",
                "co2_purchased",
                int(len(daily_co2_purchased) / DAYS_PER_YEAR),
            ),
            "yearly_co2_recycled_mscf": ProfilerUtils.resample_profile(
                daily_co2_recycled,
                "yearly",
                "co2_recycled",
                int(len(daily_co2_recycled) / DAYS_PER_YEAR),
            ),
        }
