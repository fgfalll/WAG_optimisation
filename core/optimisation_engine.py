...
# Safely import pyswarms without polluting the root logger with its default handlers
_root_logger = logging.getLogger()
_existing_handlers = list(_root_logger.handlers)
import pyswarms as ps
for _handler in _root_logger.handlers[:]:
    if _handler not in _existing_handlers:
...
class OptimizationEngine:
    ...
    def _init_simulation_engine(self):
        """Initialize the simulation engine via EngineFactory - surrogate only."""
        try:
            ...
            self.simulation_engine = EngineFactory.create_engine(
                EngineType.SURROGATE, recovery_model_type=recovery_model_type
            )
    ...
    def evaluate_for_analysis(
        ...
        if econ_params is None:
            econ_params = self.economic_params

        ooip = kwargs.get("ooip_override")
        if ooip is None:
            ooip = self.reservoir.ooip_stb
        ...
        if co2_storage_params is None:
            co2_storage_params = self.co2_storage_params
        ...
        if self.simulation_engine is not None:
            try:
                ...
                profiles = {
                    ...
                    f"{time_res}_oil_stb": get_rate("oil_production_rate") * dt,
                    f"{time_res}_gas_stb": get_rate("gas_production_rate") * dt,
                    f"{time_res}_water_stb": get_rate("water_production_rate") * dt,
                    f"{time_res}_pressure": get_rate("pressure"),
                    f"{time_res}_co2_purchased_mscf": get_rate("co2_injection") * dt,
                    f"{time_res}_co2_injected_mscf": get_rate("co2_injection") * dt,
                    f"{time_res}_water_injected_bbl": get_rate("water_injection_rate") * dt,
                    f"{time_res}_co2_produced_mscf": get_rate("gas_production_rate") * dt,  # Assume produced gas is mostly CO2
                    # Keep generic rate aliases for backward compatibility with plotters
                    "oil_production_rate": get_rate("oil_production_rate"),
                    "gas_production_rate": get_rate("gas_production_rate"),
                    "water_production_rate": get_rate("water_production_rate"),
                    "co2_injection": get_rate("co2_injection"),
                    "co2_injection_mscf": get_rate("co2_injection") * dt,
