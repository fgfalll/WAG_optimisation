...
class OptimizationEngine:
    ...
    def _objective_function_wrapper(self, **kwargs) -> float:
        ...
        pressure_data = profiles.get(
            pressure_key,
            np.array([getattr(self.eor_params, "target_pressure_psi", self.eor_params.max_pressure_psi)])
        )
        ...
        if len(pressure_data) > 0:
            ...
            avg_pressure = float(getattr(self.eor_params, "target_pressure_psi", self.eor_params.max_pressure_psi))
    ...
    def optimize_genetic_algorithm(self, ga_params_override, **kwargs) -> Dict[str, Any]:
        ...
        if final_profiles is None:
            logger.warning('No profiles returned by surrogate engine.')
            final_profiles = {}
        evaluated_points = []
